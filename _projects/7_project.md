...
## Abstract
CLOCK and SIEVE are among the most widely deployed cache replacement policies in production, mainly prized for their simplicity and frugality. Unfortunately, they are susceptible to pathologies such as \emph{performance cliffs}, wherein increasing cache capacity has little effect over a range of sizes until a workload-dependent threshold is crossed, after which hit rate improves abruptly and disproportionately. 
...
We thus introduce a randomized SIEVE/CLOCK variant that uses $\lceil \log_2 (K+1) \rceil \geq 1$ access bits per cached item, which is as cheap to implement as CLOCK, yet robust to scans and amenable to a natural CTMC description, enabling accurate performance analysis via a heterogeneous mean-field model.
    
Compared with other cache replacement algorithms, we found the randomized SIEVE/CLOCK achieves hit rates $1.5\times$ higher or more on production workloads with long scan sequences. Furthermore, though the cache hit rate improves with the number of access bits, we show that the majority of the gain is already achieved with as few as $4$ access bits per cached item.
